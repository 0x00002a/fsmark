...
generateSection (SubParser descs) = (subparser . foldMap cmdInfo) descs
  where
    ...
    cmdInfo (SubParserDesc cmd_name desc parser) =
        O.command (unpack cmd_name) (info' parser (unpack desc))
    cmdInfo (SubParserGroup name) = O.commandGroup $ unpack name

generateDescSubParser sections = generateSection $ SubParser $ map makeDesc
                                                                   sections
  where
...
shelfCommands = ShelfCmd <$> generateDescSubParser
    ...
    , ( "import"
      , "Import a shelf from a file or stdin"
      , Just $ ImportShelf <$> optionalFilePathArg
      )
    , ("export", "Export a shelf to a file or stdout", Just exportOpts)
    ]
  where
    shelfAddOpts = AddShelf <$> strArgument (metavar "NAME")
    shelfRemoveOpts =
    ...
    exportOpts =
        ExportShelf
            <$> strArgument
                    (metavar "SHELF" <> help "Name of the shelf to be exported")
            <*> optionalFilePathArg
...
mainCommands =
    generateDescSubParser
            ...
            , ( "remove"
              , "Remove an entry from the current shelf"
              , Just remove_opts
              )
  ...
  where
    ...
    pathPrintOpts   = List False . Just <$> strArgument (metavar "TARGET")
    ...
    list_opts =
        List
            <$> switch
                    ...
                    <> short 'f'
                    <> help
                           "Show full information about each entry (name, path, etc)"
                    )
            <*> optional
                    (strOption
                        (  long "search"
                        <> short 's'
                        <> help
                               "Filter results by a search term. Understands [%|*] as wildcard, i.e name* or name% will match anything starting with name, %name% or *name* will matching anything containing name"
                        )
                    )
    remove_opts = Remove <$> argument str (metavar "NAME") <*> noConfirmSwitch
        ...
        AddCmd
            ...
            <*> optional
                    (strOption
                        ...
                        <> short 'n'
                        <> help
                               "Set a name for the target (removes name confirmation dialog). Does nothing if multiple paths are specified"
                        )
                    )
            <*> noConfirmSwitch
            <*> switch (long "recursive" <> short 'r' <> help "Add recursively")
            <*> O.option
                    O.auto
                    (  long "depth"
                    <> short 'd'
                    <> help
                           "Recursion depth, no effect without -r|--recursive. Set to -1 for maximum possible"
                    <> O.value 1
                    )
...
fsmDesc =
    O.progDesc "A bookmarking system for your filesystem"
        <> O.footer
               "'fsm <command> --help' can be used to view more specific help for each command"
        ...
        ArgsResult
            ...
            <*> optional
                    (strOption
                        (  long "database"
                        <> help
                               "Path to custom database (will be created if it does not exist)"
                        )
                    )
            ...
            <*> switch
                    (  long "dry-run"
                    <> help
                           "Act as if doing things but don't actually write anything"
                    )

    internalShelfArg =
        strOption (long "shelf" <> short 's' <> help "The shelf to use")

noConfirmSwitch = switch
    (long "no-confirm" <> short 'y' <> help
...
fromToOpts target = target <$> fromArg <*> toArg
  where
    fromArg = strOption
        (long "from" <> short 'f' <> help "Source name" <> metavar "SOURCE")
    toArg = strOption
        (long "to" <> short 't' <> help "Target name" <> metavar "TARGET")
